...
values
  ...
  ('interactive_qris_payment', '{"enabled":false,"merchantName":"","qrImageUrl":"","instructions":"Scan QRIS merchant lalu bayar sesuai nominal unik yang muncul di billing.","contact":"","closedHours":{"enabled":true,"timezone":"Asia/Jakarta","start":"22:00","end":"05:00","message":"Pembayaran QRIS tutup pukul 22:00 sampai 05:00 WIB. Silakan pilih metode lain atau kembali saat jam operasional."}}'::jsonb, true, 'Konfigurasi QRIS otomatis dengan nominal unik'),
  ...
  ('ai_redraw_model', '{"mode":"standard","preset":"standard","label":"Standar Hemat","provider":"aivene_image","primaryProvider":"aivene_image","fallbackProvider":"openai_image","aiveneImageModel":"gpt-image-2","openAiImageModel":"gpt-image-2","promptProfile":"logo_photo_cleanup_short","generationQuality":"medium","imageSize":"1K","inputFidelity":"low","inputMaxEdge":1080,"backgroundMode":"transparent","safetyEnabled":true,"aspectPolicy":"match_source","resolutionPolicy":"standard","preprocess":"browser_1080_then_provider","persistPrompt":true,"retryOnLowConfidence":false,"estimatedUsdPerImage":0.04,"note":"Default AIVene GPT Image 2 dengan input maksimal 1080px, fidelity low, dan OpenAI fallback otomatis; trace berjalan lokal di browser."}'::jsonb, false, 'Pipeline AIVene GPT Image 2 + OpenAI fallback, trace lokal di browser')
on conflict (key) do update
set value = case
