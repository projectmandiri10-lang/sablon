...
values
  ...
  ('ai_redraw_model', '{"mode":"quality","preset":"quality","label":"Kualitas","provider":"gemini_direct_image","primaryProvider":"gemini_direct_image","fallbackProvider":"openrouter_image","geminiGenerationModel":"gemini-3.1-flash-image","geminiReasoningModel":"gemini-2.5-pro","geminiFallbackPolicy":"quota_or_model_unavailable","analysisModel":"","generationModel":"black-forest-labs/flux.2-klein-4b","fallbackModel":"sourceful/riverflow-v2-fast","safetyModel":"nvidia/nemotron-3.5-content-safety:free","promptProfile":"generic_trace_clone","generationQuality":"high","imageSize":"1K","reasoningEffort":"medium","backgroundMode":"transparent","safetyEnabled":true,"aspectPolicy":"match_source","resolutionPolicy":"high","preprocess":"node_heuristic","persistPrompt":true,"retryOnLowConfidence":false,"estimatedUsdPerImage":0.05,"note":"Default Gemini direct 1K trace-clone dengan OpenRouter fallback otomatis saat quota, billing, atau model unavailable."}'::jsonb, false, 'Pipeline Gemini direct redraw primary + OpenRouter fallback untuk AI redraw')
on conflict (key) do update
set value = case
